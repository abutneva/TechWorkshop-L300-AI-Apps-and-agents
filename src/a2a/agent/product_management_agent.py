...
class AgentFrameworkProductManagementAgent:
    ...
    async def invoke(self, user_input: str, session_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            'content': response.text if response.text else 'Unable to process request.'
        }
    ...
    async def stream(
        ...
        if chunks:
            # Combine all chunks and return as plain text
            combined_text = sum(chunks[1:], chunks[0]) if len(chunks) > 1 else chunks[0]
            yield {
                'is_task_complete': False,
                'require_user_input': True,
                'content': combined_text if combined_text else 'Unable to process request.'
            }
